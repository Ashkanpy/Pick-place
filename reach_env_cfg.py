...
class ActionsCfg:
    ...
    # Replace the JointPositionActionCfg with a DIK action that tracks the ee_pose command
    arm_action: ActionTerm = DifferentialInverseKinematicsActionCfg(
        asset_name="robot",
        joint_names=[".*_joint"],
        body_name="ee_link",
        #command_name="ee_pose",               # follows CommandsCfg.ee_pose
...
class EventCfg:
    ...
    auto_close_gripper = EventTerm(
        func=mdp.close_gripper_when_near_command,
        mode="interval",
        interval_range_s=(0.0, 0.0),
        params={
            # also resolve the gripper joint id here
            "asset_cfg": SceneEntityCfg("robot", body_names=["ee_link"], joint_names=["finger_joint"], preserve_order=True),
        },
    )
